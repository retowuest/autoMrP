ebma <- function(ebma.fold, L1.x, L2.x, L2.unit, L2.reg, post.strat, Ndraws = 100, 
  ...
  names(final_model_weights) <- c("BestSubset", "PCA", "Lasso")
  
  # weighted average
  L2_preds <- post.strat$predictions$Level2 %>%
    dplyr::mutate(ebma = as.numeric(cbind(best_subset, pca, lasso) %*% final_model_weights))
