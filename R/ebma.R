ebma <- function(ebma.fold, L1.x, L2.x, L2.unit, L2.reg, post.strat, Ndraws = 100, 
                 ...
                 best.subset, pca, lasso, gb, verbose){
  ...
  names(final_model_weights) <- c("BestSubset", "PCA", "Lasso", "GB")
  
  # weighted average
  L2_preds <- post.strat$predictions$Level2 %>%
    dplyr::mutate(ebma = as.numeric(cbind(best_subset, pca, lasso, gb) %*% final_model_weights))
