  ...
  if (sum(unlist(lapply(X = post.strat$models, FUN = function(x) !is.null(x)))) > 1){
        ...
        for (idx.Ndraws in 1:n.draws) {
          ...
          test_preds <- dplyr::tibble(
            ...
            lasso = if (!is.null(model_lasso)) {
              predict_glmmLasso(
                census = test, m = model_lasso, L1.x = L1.x,
          ...
          test_preds <- test_preds[,
            apply(X = test_preds, MARGIN = 2, FUN = function(x) {
              all(!is.na(x))})]
          ...
          if (verbose) cat(paste("\n", "EBMA: ", round(
            counter / (length(tol) * n.draws), 2) * 100, "% done", sep = ""))
        }
      }
    ...
    L2_preds <- dplyr::tibble(
      !! rlang::sym(L2.unit) := dplyr::pull(
        .data = post.strat$predictions$Level2,
        var = L2.unit),
      ebma = w_avg)
...
#'   data. A tibble.
#' @param train.y Outcome variable of the classifier training data. A
#'   numeric vector.
...
ebma_mc_tol <- function(
  ...
  for(idx.Ndraws in 1:n.draws){
    ...
    weights <- matrix(
      ...
      byrow = TRUE,
      dimnames = list(c(paste0("Tol_", tol)), c(names(train.preds))))
